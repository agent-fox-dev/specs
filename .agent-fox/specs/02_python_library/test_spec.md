...
**Type:** integration
**Description:** Verify load-save-load produces byte-identical files and equivalent in-memory state.
...
**Expected:**
- After load → save → load, files are byte-identical and in-memory state is equivalent.
...
**Type:** property
**Description:** Loading and saving any valid spec produces byte-identical files.
...
**Invariant:** `load(save(load(path))) == load(path)` — files on disk are byte-identical after a round-trip.
...
### TS-02-E22: EARS rendering omits null return_contract

**Requirement:** 02-REQ-6.E2
**Type:** unit
**Description:** Verify null return_contract is omitted from rendered output.
...
**Expected:**
- No return contract clause in rendered sentence.

**Assertion pseudocode:**
```
criterion = EventDrivenCriterion(id="05-REQ-1.1", ..., return_contract=None)
rendered = render_ears(criterion)
